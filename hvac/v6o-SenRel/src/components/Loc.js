    ...
    const maybeLoad=()=>{
      switch(true){
        ...
        case status=='success':
          ...
          if(params.sr){
            return(<Zones devs={this.devso} zones={this.zoneso.filter((z)=>z.id==params.sr)} {...this.state} {...this.props.cambio}/>)
          }else{
            return(<Zones devs={this.devso} zones={this.zoneso} {...this.state} {...this.props.cambio}/>)
          }
        default:
          return (<p>{message}</p>)
      }
