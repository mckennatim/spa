...
class SensorRelay extends React.Component{
  ...
  componentDidMount(){
    ...
    this.unsub = hookupMqtt(this.loc, ls, (qstate)=>{
      ...
      qdata: {kid:{id, img. name, sched:[[]],
                  schedobj:{dow, idx, now, sched:[{time, setpt},]}
                  spec:{type, diff}, 
                  sr:{temp,relay,setpt,timeleft},
                  ts},},
      readystae:{ready,message},
      tz
      }
      let prior = {urlsr:'', message:''}

      when message changes
      when url changes setState qdata[urlsr]
      filter for urlsr 
      when state changes for dev, id (prior is )
      */
      ...
      // let cur = reset.stateWhenUrlsrChanges(prior, urlsr, qstate, this, (cur)=>{
      //   console.log('res urlsr')
      //   if(this.state.qdata){
      //     let newstate = {...this.state}
      //     newstate.qdata.sr = cur.srrec
      //     const schedobj = createSchedObj(cur.sched, this.state.tz)
      //     newstate.schedobj = schedobj
      //     newstate.dow = schedobj.dow
  ...
  render(){
    console.log('this.props.cambio: ',JSON.stringify(this.props.cambio))
 
    console.log('this.sr: ',this.sr)
 
    if (this.state.qdata) {
      console.log(this.state.qdata.id)
