...
import {covertSchedFromDb} from '../services/schedobj'
...
class WeekSched extends React.Component{
  ...
  componentDidMount(){
    ...
    this.unsub = hookupMqtt(this.loc, ls, (qstate)=>{
      ...
      // status.prior.sr.dev=qstate.qdata[urlsr].sr.dev
      // if(status.prior.sr.id != qstate.qdata[urlsr].sr.id){
      //   console.log('Shit ahs changed')
      //   this.setState({qdata:qstate.qdata[urlsr]})
      //   let qu = qstate.qdata[urlsr]
      //   let dev = qu.sr.dev
      ...
      //   }
      // }
      // status.prior.sr.id=qstate.qdata[urlsr].sr.id
      //status = reset.ck4changeWeek(qstate.qdata[urlsr], status)
      // let qu = qstate.qdata[urlsr]
      // //console.log('status: ',JSON.stringify(status))
      // if(ischanged){
      //   console.log('STATUS changed',JSON.stringify(qu.sr))
      //   this.setState({qdata:qstate.qdata[urlsr]})
      //   let dev = qu.sr.dev
      //   let id = qu.sr.id
      //   if(dev.length>5){
      ...
      fetch(url, options)
        ...
        .then((json)=>{
          if(json.message){
            ...
            console.log(covertSchedFromDb(json))
          }
        })   
    }else{
    ...
    if (this.state.qdata) {
      ...
      return (
        ...
        <div>
          ...
          <Sched sched={this.state.cursched} idx={99}/>
        </div>
