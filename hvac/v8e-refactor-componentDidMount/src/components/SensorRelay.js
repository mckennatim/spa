...
class SensorRelay extends React.Component{
  ...
  componentDidMount(){
    ...
    this.unsub = hookupMqtt(this.loc, ls, (qstate)=>{
      ...
      reset.stateYtimerWhenSchedChanges(prior.sched, cur.sched, this)
  ...
  resetTimer = (fro)=>{
    let mom = moment(fro,'H:mm')
    // console.log('resetting timr to: ',mom.format('H:mm'))
    this.setState({fornext: mom.format('H:mm')})
    if (this.tmr){
      clearInterval(this.tmr)
    }
    this.tmr = setInterval(()=>{
      mom.subtract(60, 'seconds')
      // console.log(mom.format('H:mm'))
      this.setState({fornext: mom.format('H:mm')})
    },60000)
  }
  ...
  handleDaySched =()=>{
    router.navigate('sched/12ParleyVale/lr/5')
  }

  render(){
    if (this.state.qdata) {
      //console.log(this.state.qdata)
      //console.log('this.state.schedobj.timeleft: ',JSON.stringify(this.state.schedobj.timeleft))
      ...
      const displaySchedule=()=>{
        if(this.state.schedobj){
          //console.log(this.state.schedobj)
