...
class Persons extends React.Component{
  ...
  state={
    persons: [{person: 'duck', id: 99, effective:'2018-01-01T14:12'}],
    wk: moment().week(),
    filt: 'all',
  ...
  editPerson=(j)=>{
    console.log('j: ', j)
    let jo = {person:j.person, active:j.active, idx:j.idx, week:0}
    let ar = []
    this.state.persons
      .filter((person)=>person.idx==j.idx)
  ...
  renderPersons=()=>{
    ...
    const rpersons = persons
      .map((aperson)=>{
        ...
        return (
        <li  key={aperson.id} style={style.myli.li}>
          ...
          <div style={style.myli.act}>
            <input style={style.myli.ck} type="checkbox" checked= {aperson.active} onChange={this.onChecked.bind(null, aperson)}></input>
            </div>
        </li >)
    })
    return(
