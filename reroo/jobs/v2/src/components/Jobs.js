...
class Jobs extends React.Component{
  ...
  sav2wk = ()=>{
    console.log(this.dwk.value);
    const jobs = this.state.jobs
      .filter((j)=>j.active)
      .map((j)=>{return {job: j.job, category: j.category,   active: j.active*1, idx: j.idx, week:this.dwk.value, coid:j.coid}})
    postJobs(jobs, this.dwk.value)  
  }
  sav = () =>{
